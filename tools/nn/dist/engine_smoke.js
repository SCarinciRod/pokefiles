...
function assertEq(actual, expected, label) {
    ...
    assert(actual === expected, `${prefix}expected ${JSON.stringify(expected)}, got ${JSON.stringify(actual)}`);
}
function assertApprox(actual, expected, tolerance, label) {
    const prefix = label ? `${label}: ` : '';
    assert(Math.abs(actual - expected) <= tolerance, `${prefix}expected ≈${expected} (±${tolerance}), got ${actual}`);
}
// ---------------------------------------------------------------------------
// 1. Golden Pokédex — 20 Pokémon spot-checks
// ---------------------------------------------------------------------------
const GOLDEN_POKEMON = [
...
// ---------------------------------------------------------------------------
function makePokemonProfile(identifier, speed, priority = 0, damage = 100) {
    const stats = (0, engine_1.createEmptyStatMap)(1);
    stats.speed = speed;
...
// Run tests
// ---------------------------------------------------------------------------
function runPokedexTests(engine) {
    ...
    for (const golden of GOLDEN_POKEMON) {
        test(`${golden.name} — types`, () => {
            ...
            for (const expectedType of golden.types) {
                assert(p.types.includes(expectedType), `expected type ${expectedType}, got [${p.types.join(', ')}]`);
            }
        });
        ...
        test(`${golden.name} — abilities`, () => {
            ...
            for (const ability of golden.abilities) {
                assert(p.abilities.includes(ability), `expected ability ${ability}, got [${p.abilities.join(', ')}]`);
            }
        });
    }
}
function runDamageTests(engine) {
    ...
    for (const check of DAMAGE_CHECKS) {
        test(check.label, () => {
            ...
            assert(profile.avg >= profile.min && profile.avg <= profile.max, `avg (${profile.avg}) must be between min and max`);
        });
    }
}
function runVGCTests() {
    console.log('\n[3] VGC turn order');
    test('Faster Pokémon moves first (no field effects)', () => {
        const sc = (0, engine_1.createSpeedControlState)();
        const fast = makePokemonProfile('fast', 100);
        const slow = makePokemonProfile('slow', 50);
        const order = (0, engine_1.resolveTurnOrder)([slow, fast], sc);
        assertEq(order[0].identifier, 'fast', 'first mover');
    });
    test('Trick Room reverses speed order', () => {
        const sc = (0, engine_1.activateTrickRoom)((0, engine_1.createSpeedControlState)());
        const fast = makePokemonProfile('fast', 100);
        const slow = makePokemonProfile('slow', 50);
        const order = (0, engine_1.resolveTurnOrder)([fast, slow], sc);
        assertEq(order[0].identifier, 'slow', 'slow should move first under TR');
    });
    test('Tailwind doubles effective speed', () => {
        const sc = (0, engine_1.activateTailwind)((0, engine_1.createSpeedControlState)());
    ...
    test('Trick Room does not override priority bracket', () => {
        ...
        const order = (0, engine_1.resolveTurnOrder)([fastNormal, slowPriority], sc);
        assertEq(order[0].identifier, 'slow_priority', 'priority still wins under TR');
    });
    test('Trick Room turns decrement correctly', () => {
        let sc = (0, engine_1.activateTrickRoom)((0, engine_1.createSpeedControlState)());
    ...
    test('Re-activating Trick Room cancels it', () => {
        let sc = (0, engine_1.activateTrickRoom)((0, engine_1.createSpeedControlState)());
        sc = (0, engine_1.activateTrickRoom)(sc);
        assertEq(sc.trickRoomTurns, 0, 'should be cancelled');
    });
}
function runTypeTests(engine) {
    console.log('\n[4] Type effectiveness key matchups');
    const chart = engine.getTypeChart();
    function typeCheck(attack, defense, expected) {
...
function runBridgeTests() {
    ...
    function sendBridgeCommand(command) {
        const result = (0, child_process_1.spawnSync)('npx', ['ts-node', 'bridge.ts'], {
            input: command + '\n',
...
function main() {
    if (!fs.existsSync(DB_PATH)) {
        ...
        console.error('[smoke] Run the data pipeline: node tools/generate_generation_db.js all (and other generate_*.js scripts)');
        process.exit(1);
    }
    console.log(`[smoke] db=${DB_PATH}`);
    const engine = new engine_1.DeterministicEngine(DB_PATH);
    try {
