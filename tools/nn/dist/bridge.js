...
const nnMoveArchetypeMap = new Map();
let pyProcess = null;
let nnReady = false;
const nnPendingRequests = new Map();
let nnReqIdCounter = 0;
function nnAbilityArchetypes(abilities) {
    return NN_ABILITY_ARCHETYPES.map((set) => abilities.some((a) => set.has(a)) ? 1 : 0);
}
function nnRoleOneHot(stats) {
    const s = stats;
    let roleIdx = 6; // support_utility
    if (s.attack >= 100 && s.speed >= 85 && s.special_attack < 90)
        roleIdx = 0;
    else if (s.special_attack >= 100 && s.speed >= 85 && s.attack < 90)
        roleIdx = 1;
    else if (s.defense >= 100 && s.hp >= 90)
        roleIdx = 2;
    else if (s.special_defense >= 100 && s.hp >= 90)
        roleIdx = 3;
    else if (s.hp >= 100 && s.defense >= 80 && s.special_defense >= 80)
        roleIdx = 4;
    else if (s.speed >= 110)
        roleIdx = 5;
...
function buildNNMoveArchetypeMap(engine) {
    const rows = engine.queryAll(`SELECT pokemon_id, move_id FROM pokemon_moves WHERE pokemon_id IS NOT NULL`);
    for (const { pokemon_id, move_id } of rows) {
        if (!nnMoveArchetypeMap.has(pokemon_id))
            nnMoveArchetypeMap.set(pokemon_id, new Array(8).fill(0));
        const flags = nnMoveArchetypeMap.get(pokemon_id);
        for (let i = 0; i < NN_MOVE_ARCHETYPES.length; i++) {
            if (NN_MOVE_ARCHETYPES[i].has(move_id))
                flags[i] = 1;
        }
    }
    process.stderr.write(`[bridge] nn move archetype map: ${nnMoveArchetypeMap.size} pokemon\n`);
}
function startNNProcess() {
    ...
    return new Promise((resolve) => {
        pyProcess = (0, child_process_1.spawn)(pythonCmd, [NN_SCRIPT_PATH], {
            ...
            for (const line of lines) {
                ...
                try {
                    ...
                    if (msg['type'] === 'ready') {
                        ...
                        const cb = nnPendingRequests.get(msg['id']);
                        if (cb) {
                            cb(msg['score']);
                            nnPendingRequests.delete(msg['id']);
                        }
                    }
                }
        ...
        pyProcess.on('exit', (code) => {
            ...
            for (const [, cb] of nnPendingRequests)
                cb(0.5);
            nnPendingRequests.clear();
        });
        // Timeout: if model not ready in 20s, continue without NN
        setTimeout(() => { if (!nnReady) {
            process.stderr.write('[bridge] nn: startup timeout — heuristics only\n');
            resolve();
        } }, 20000);
    });
}
function queryNN(features) {
    if (!nnReady || !pyProcess)
    ...
    return new Promise((resolve) => {
        ...
        setTimeout(() => {
            if (nnPendingRequests.has(id)) {
                nnPendingRequests.delete(id);
                resolve(0.5);
            }
        }, 5000);
    });
...
let nluProcess = null;
let nluReady = false;
const nluPendingRequests = new Map();
let nluReqIdCounter = 0;
function startNLUProcess() {
    const pythonCmd = process.platform === 'win32' ? 'python' : 'python3';
    ...
    return new Promise((resolve) => {
        nluProcess = (0, child_process_1.spawn)(pythonCmd, [NLU_SCRIPT_PATH], {
            cwd: process.env['BRIDGE_PY_CWD'] ?? path.join(__dirname, '..', '..'),
            stdio: ['pipe', 'pipe', 'pipe'],
        });
        let buf = '';
        nluProcess.stdout.on('data', (chunk) => {
            buf += chunk.toString();
            const lines = buf.split('\n');
            buf = lines.pop() ?? '';
            for (const line of lines) {
                ...
                try {
                    ...
                    if (msg['type'] === 'ready') {
                        ...
                        const cb = nluPendingRequests.get(msg['id']);
                        if (cb) {
                            cb({ intent: msg['intent'], confidence: msg['confidence'] ?? 0 });
                            nluPendingRequests.delete(msg['id']);
                        }
                    }
                }
                catch { /* ignore malformed lines */ }
        ...
        nluProcess.on('exit', (code) => {
            ...
            for (const [, cb] of nluPendingRequests)
                cb({ intent: 'unknown', confidence: 0 });
            nluPendingRequests.clear();
        });
        // Timeout: if not ready in 30s, continue without NLU
        setTimeout(() => {
            if (!nluReady) {
                process.stderr.write('[bridge] nlu: startup timeout — regex-only\n');
                resolve();
            }
        }, 30000);
    });
}
function queryNLU(text) {
    if (!nluReady || !nluProcess)
        return Promise.resolve({ intent: 'unknown', confidence: 0 });
...
function compStat(base, isHp, evs, boosted) {
    const IV = 31, level = 50;
    if (isHp)
        return Math.floor(Math.floor((2 * base + IV + Math.floor(evs / 4)) * level / 100) + level + 10);
...
function compSpeed(base) { return compStat(base, false, 252, true); }
// Max competitive offense (252 EVs, +Atk/SpAtk nature)
function compOffense(base) { return compStat(base, false, 252, true); }
// Defensive EVs without nature boost
function compDef(base, evs = 252) { return compStat(base, false, evs, false); }
// HP at 252 EVs
function compHP(base) { return compStat(base, true, 252, false); }
...
function typeLabel(t) { return t ? (TYPE_LABELS[t] ?? t) : ''; }
...
function isLegendary(identifier) {
    if (LEGENDARY_BASES.has(identifier))
        return true;
...
function buildNfeSet(engine) {
    nfeIds.clear();
    for (const { identifier } of engine.queryAll('SELECT p.identifier FROM pokemon p WHERE p.id IN (SELECT DISTINCT from_id FROM pokemon_evolution)')) {
        nfeIds.add(identifier);
    }
}
function isNfe(identifier) {
    if (nfeIds.has(identifier))
        return true;
...
function buildNameIndex(allPokemon) {
    nameIndex.clear();
...
function levenshtein(a, b) {
    ...
    if (n === 0)
        return m;
    let prev2 = new Array(n + 1).fill(0); // dp[i-2][*]
    let prev1 = Array.from({ length: n + 1 }, (_, i) => i); // dp[i-1][*]
    const curr = new Array(n + 1).fill(0); // dp[i][*]
    for (let i = 1; i <= m; i++) {
        curr[0] = i;
        for (let j = 1; j <= n; j++) {
...
function fuzzyMatchToken(token, exclude = []) {
    if (token.length < 4)
        return null;
    ...
    for (const key of sortedKeys) {
        ...
        if (Math.abs(token.length - key.length) > maxDist)
            continue;
...
function buildAbilityIndex(engine) {
    abilityIndex.clear();
    for (const { id } of engine.queryAll('SELECT id FROM abilities')) {
        abilityIndex.set(id, id);
        abilityIndex.set(id.replace(/_/g, ''), id);
        abilityIndex.set(id.replace(/_/g, ' '), id);
    }
}
function buildMoveIndex(engine) {
    moveIndex.clear();
    for (const { id } of engine.queryAll('SELECT id FROM moves')) {
        moveIndex.set(id, id);
        moveIndex.set(id.replace(/_/g, ''), id);
        moveIndex.set(id.replace(/_/g, ' '), id);
    }
}
function extractAbilityName(t) {
    const sorted = [...abilityIndex.keys()].sort((a, b) => b.length - a.length);
    for (const key of sorted) {
        if (t.includes(key))
            return abilityIndex.get(key);
    }
    return null;
}
function extractMoveName(t) {
    const sorted = [...moveIndex.keys()].sort((a, b) => b.length - a.length);
    for (const key of sorted) {
        if (t.includes(key))
            return moveIndex.get(key);
    }
    return null;
}
// ---------------------------------------------------------------------------
...
function hasTypeToken(t) { return extractTypeName(t) !== null; }
function extractGenerationNumber(t) {
    ...
    const words = { um: 1, dois: 2, tres: 3, quatro: 4, cinco: 5, seis: 6, sete: 7, oito: 8, nove: 9, one: 1, two: 2, three: 3, four: 4, five: 5, six: 6, seven: 7, eight: 8, nine: 9 };
    for (const [word, num] of Object.entries(words)) {
        if (t.includes(word) && /ger[ae]?|gen/.test(t))
            return num;
    }
    const regions = { kanto: 1, johto: 2, hoenn: 3, sinnoh: 4, unova: 5, kalos: 6, alola: 7, galar: 8, paldea: 9 };
...
function extractStatNameFromText(t) {
    if (/\bhp\b|pontos.*vida|mais.*vida/.test(t))
        return 'hp';
    if (/ataque.*esp|spatk|sp.*atk|especial.*ataque/.test(t))
        return 'special_attack';
    if (/defesa.*esp|spdef|sp.*def|especial.*defesa/.test(t))
        return 'special_defense';
    if (/\bataque\b|attack\b/.test(t))
        return 'attack';
    if (/\bdefesa\b|defense\b/.test(t))
        return 'defense';
    if (/\bbst\b|total.*base|poder.*base/.test(t))
        return 'bst';
    return 'speed';
}
function detectExcludeLegendary(t) {
...
// ---------------------------------------------------------------------------
function classifyRole(stats, types) {
    ...
    if (speed >= 110 && mainOff >= 115)
        return `Sweeper ${offBias} extremo`;
    if (speed >= 90 && mainOff >= 95)
        return `Sweeper ${offBias}`;
    if (speed >= 100 && mainOff < 75 && (hp + defense + spDef) >= 220)
        return 'Speed Control / Pivot';
    if (speed >= 100 && mainOff < 75)
        return 'Pivot / Disruptor';
    if (speed <= 45 && hp >= 80)
        return 'Trick Room Abuser';
    if (bulkScore >= 20000 && speed <= 65)
        return `Tanque ${offBias}`;
    if (hp >= 80 && mainOff >= 90 && speed >= 70)
        return `Attacker Bulky ${offBias}`;
    if (types.includes('fairy') || types.includes('psychic')) {
        if (spAtk >= 90)
            return `Sweeper ${offBias} / Suporte`;
    }
    return `Utility / Suporte ${offBias}`;
}
function speedTierLabel(baseSpeed) {
...
// Intent handlers
// ---------------------------------------------------------------------------
function handleCompetitiveProfile(identifier, engine) {
    const p = engine.getPokemonContext(identifier);
    if (!p)
        return `Bot: Pokémon "${displayName(identifier)}" não encontrado na base de dados.`;
    ...
    const moveset = new Set(engine.queryAll('SELECT DISTINCT move_id FROM pokemon_moves WHERE pokemon_identifier = ?', [identifier])
        .map((r) => r.move_id));
    ...
    for (const [atkType, byAtk] of typeChart) {
        ...
        if (mult >= 2.0)
            weaknesses.push(`${typeLabel(atkType)}${mult >= 4 ? '×4' : ''}`);
    }
    const primaryStabMoves = [];
    for (const stabType of p.types) {
        ...
        const best = engine.queryAll(`SELECT m.id, m.base_power, m.category
       ...
       ORDER BY m.base_power DESC LIMIT 5`, [identifier, stabType]).filter((m) => !STAB_MOVE_BLACKLIST.has(m.id)).slice(0, 2);
        for (const m of best) {
    ...
    for (const abilityId of p.abilities) {
        ...
        if (passive) {
            ...
            if (cond.speedMult)
                note += ` → Vel ${speed} × ${cond.speedMult} = ${speed * cond.speedMult}`;
            else if (cond.atkMult)
                note += ` → Atq×${cond.atkMult}`;
            lines.push(`  ⚡ ${note}`);
        }
        else if (status) {
    ...
    if (hasMoveInfo) {
        ...
        needs.setupMoves.forEach((sm) => lines.push(`  ◆ Setup: ${displayName(sm.move)} — ${sm.desc} (precisa de 1 turno protegido)`));
...
function handleSpeedRanking(excludeLegendary, limit, engine) {
    return handleStatRanking('speed', excludeLegendary, limit, engine);
}
function handleStatRanking(statId, excludeLegendary, limit, engine) {
    let rows;
    if (statId === 'bst') {
        rows = engine.queryAll(`SELECT p.identifier, SUM(ps.value) AS value
       FROM pokemon p JOIN pokemon_stats ps ON p.id = ps.pokemon_id
       GROUP BY p.id ORDER BY value DESC`);
    }
    else {
        rows = engine.queryAll(`SELECT p.identifier, ps.value AS value
       FROM pokemon p JOIN pokemon_stats ps ON p.id = ps.pokemon_id
       WHERE ps.stat_id = ? ORDER BY ps.value DESC`, [statId]);
    }
    let filtered = rows.filter((r) => !/_mega|_gmax/.test(r.identifier));
    if (excludeLegendary)
        filtered = filtered.filter((r) => !isLegendary(r.identifier));
...
function handleTypeQuery(typeId, genFilter, excludeLegendary, engine) {
    ...
    const params = [typeId];
    if (genFilter !== null) {
        sql += ' AND p.source_generation = ?';
        params.push(genFilter);
    }
    sql += ' GROUP BY p.id ORDER BY p.id';
    let rows = engine.queryAll(sql, params);
    rows = rows.filter((r) => !/_mega|_gmax/.test(r.identifier));
    if (excludeLegendary)
        rows = rows.filter((r) => !isLegendary(r.identifier));
...
function handleGenerationQuery(generation, typeFilter, engine) {
    let sql = `SELECT p.identifier, p.source_generation,
       GROUP_CONCAT(pt.type_id, '/') AS types
     FROM pokemon p JOIN pokemon_types pt ON p.id = pt.pokemon_id
    ...
    if (typeFilter) {
        sql += ' AND p.id IN (SELECT pokemon_id FROM pokemon_types WHERE type_id = ?)';
...
function handleLegendaryQuery(genFilter, typeFilter, engine) {
    ...
    const rows = engine.queryAll(sql, params);
    const legendaries = rows.filter((r) => isLegendary(r.identifier));
    if (!legendaries.length)
        return `Bot: Nenhum lendário/mítico encontrado${genFilter ? ` na Geração ${genFilter}` : ''}${typeFilter ? ` do tipo ${typeLabel(typeFilter)}` : ''}.`;
    const genPart = genFilter ? ` — Geração ${genFilter}` : '';
    ...
    if (genFilter || legendaries.length <= 20) {
        ...
        const byGen = new Map();
        for (const r of legendaries) {
            if (!byGen.has(r.source_generation))
                byGen.set(r.source_generation, []);
...
function handleEvolutionChain(identifier, engine) {
    const baseRow = engine.queryAll('SELECT id FROM pokemon WHERE identifier = ?', [identifier]);
    if (!baseRow.length)
        return `Bot: Pokémon "${displayName(identifier)}" não encontrado.`;
    const startId = baseRow[0].id;
    const allEvo = engine.queryAll('SELECT from_id, to_id, trigger, min_level, condition FROM pokemon_evolution');
    const allPokemon = engine.queryAll('SELECT id, identifier FROM pokemon');
    const idToName = new Map(allPokemon.map((p) => [p.id, p.identifier]));
    const childrenMap = new Map();
    const parentOf = new Map(); // to_id → from_id (first parent only)
    for (const e of allEvo) {
        if (!childrenMap.has(e.from_id))
            childrenMap.set(e.from_id, []);
        childrenMap.get(e.from_id).push(e);
        if (!parentOf.has(e.to_id))
    ...
    const lines = [];
    const seen = new Set();
    function renderNode(id, depth, from) {
        if (seen.has(id))
            return;
        seen.add(id);
        ...
        let evoInfo = '';
        if (from) {
            if (from.trigger === 'level_up' && from.min_level)
                evoInfo = ` [Nível ${from.min_level}]`;
            else if (from.trigger === 'trade')
                evoInfo = from.condition ? ` [Troca c/ ${displayName(from.condition)}]` : ' [Troca]';
            else if (from.trigger === 'use_item')
                evoInfo = from.condition ? ` [${displayName(from.condition)}]` : ' [Item]';
            else if (from.condition)
                evoInfo = ` [${displayName(from.condition)}]`;
            else if (from.trigger)
                evoInfo = ` [${from.trigger.replace(/_/g, ' ')}]`;
        }
        lines.push(`${indent}${depth > 0 ? '→ ' : ''}${name}${evoInfo}${id === startId ? ' ◄' : ''}`);
        for (const child of (childrenMap.get(id) ?? []))
            renderNode(child.to_id, depth + 1, child);
    }
    renderNode(rootId, 0);
    if (lines.length === 1)
        lines.push('  (Não evolui)');
    return `Bot: Cadeia de evolução de ${displayName(identifier)}:\n${lines.join('\n')}`;
}
function handleCounterQuery(identifier, engine) {
    const p = engine.getPokemonContext(identifier);
    if (!p)
        return `Bot: Pokémon "${displayName(identifier)}" não encontrado.`;
    const typeChart = engine.getTypeChart();
    ...
    for (const [atkType, byAtk] of typeChart) {
        ...
        if (mult > 1.0)
            effectiveTypes.push({ type: atkType, mult });
    }
    ...
    const candidates = engine.queryAll(`SELECT DISTINCT p2.identifier,
     ...
     GROUP BY p2.id`, effectiveTypeIds);
    const scored = candidates.map((c) => {
        const defTypes = [c.type1, c.type2].filter(Boolean);
        let stabMult = 0;
        for (const et of effectiveTypes) {
            if (defTypes.includes(et.type))
                stabMult = Math.max(stabMult, et.mult);
        }
...
function handleBadMatchups(identifier, engine) {
    const p = engine.getPokemonContext(identifier);
    if (!p)
        return `Bot: Pokémon "${displayName(identifier)}" não encontrado.`;
    const typeChart = engine.getTypeChart();
    ...
    for (const { type, mult } of weaknesses) {
        ...
        const threats = engine.queryAll(`SELECT p2.identifier,
        COALESCE((SELECT value FROM pokemon_stats WHERE pokemon_id=p2.id AND stat_id='speed'),60) AS speed,
        MAX(
    ...
    for (const { type } of weaknesses.slice(0, 3)) {
        ...
        if (!byAtk)
            continue;
        const immuneDefTypes = [];
        for (const [defType, val] of byAtk) {
            if (val === 0)
                immuneDefTypes.push(defType);
        }
...
function handleCompareQuery(idA, idB, engine) {
    const a = engine.getPokemonContext(idA);
    const b = engine.getPokemonContext(idB);
    if (!a)
        return `Bot: ${displayName(idA)} não encontrado.`;
    if (!b)
        return `Bot: ${displayName(idB)} não encontrado.`;
    const STAT_ORDER = [
    ...
    if (multAvsB !== 1.0)
        lines.push(`Tipo de ${nameA} contra ${nameB}: ×${multAvsB}`);
    if (multBvsA !== 1.0)
        lines.push(`Tipo de ${nameB} contra ${nameA}: ×${multBvsA}`);
    lines.push(`Vantagem de stats: ${aWins > bWins ? nameA : bWins > aWins ? nameB : 'Empate'} (${aWins}×${bWins})`);
    return lines.join('\n');
}
function handleAbilityInfo(abilityId, engine) {
    const ab = engine.queryAll('SELECT id, short_effect, effect FROM abilities WHERE id = ?', [abilityId]);
    if (!ab.length)
        return `Bot: Habilidade "${displayName(abilityId)}" não encontrada.`;
    const eff = engine.queryAll('SELECT category, trigger, description FROM ability_effects WHERE ability_id = ?', [abilityId]);
    const lines = [
    ...
    const pokemon = engine.queryAll(`SELECT p.identifier FROM pokemon p JOIN pokemon_abilities pa ON p.id = pa.pokemon_id
     WHERE pa.ability_id = ? AND p.identifier NOT LIKE '%_mega%' AND p.identifier NOT LIKE '%_gmax%'
     ORDER BY pa.slot, p.id LIMIT 5`, [abilityId]);
    if (pokemon.length)
        lines.push(`Possui: ${pokemon.map((p) => displayName(p.identifier)).join(', ')}`);
    return lines.join('\n');
}
function handleAbilityInfoForPokemon(identifier, engine) {
    const p = engine.getPokemonContext(identifier);
    if (!p)
        return `Bot: Pokémon "${displayName(identifier)}" não encontrado.`;
    const lines = [`Bot: Habilidades de ${displayName(identifier)}:`];
    for (const abilityId of p.abilities) {
        const ab = engine.queryAll('SELECT short_effect FROM abilities WHERE id = ?', [abilityId]);
        const effect = ab.length ? (ab[0].short_effect || '') : '';
        lines.push(`  • ${displayName(abilityId)}: ${effect.slice(0, 100) || '(sem descrição)'}`);
    }
    return lines.join('\n');
}
function handleMoveInfo(moveId, engine) {
    const move = engine.queryAll('SELECT id, type_id, category, base_power, accuracy, pp, description FROM moves WHERE id = ?', [moveId]);
    if (!move.length)
        return `Bot: Golpe "${displayName(moveId)}" não encontrado.`;
    const m = move[0];
    const eff = engine.queryAll('SELECT description FROM move_effects WHERE move_id = ?', [moveId]);
    const catLabels = { physical: 'Físico', special: 'Especial', status: 'Status' };
...
function handleMovelist(identifier, engine) {
    const p = engine.getPokemonContext(identifier);
    if (!p)
        return `Bot: Pokémon "${displayName(identifier)}" não encontrado.`;
    const moves = engine.queryAll(`SELECT DISTINCT pm.move_id, m.type_id, m.category, m.base_power, mtr.role
     ...
     LIMIT 60`, [identifier]);
    if (!moves.length)
        return `Bot: Nenhum golpe encontrado para ${displayName(identifier)}.`;
...
function handleTypeCoverage(targetType, engine) {
    const typeChart = engine.getTypeChart();
    const superEffective = [], notVery = [], immune = [];
    for (const [atkType, byAtk] of typeChart) {
        const mult = byAtk.get(targetType);
        if (mult === undefined)
            continue;
        if (mult >= 2)
            superEffective.push(typeLabel(atkType));
        else if (mult <= 0)
            immune.push(typeLabel(atkType));
        else if (mult < 1)
            notVery.push(typeLabel(atkType));
    }
    const label = typeLabel(targetType);
    const lines = [`Bot: Cobertura contra o tipo ${label}:`];
    if (superEffective.length)
        lines.push(`✓ Super-efetivo (×2): ${superEffective.join(', ')}`);
    if (notVery.length)
        lines.push(`✗ Não muito efetivo (×0.5): ${notVery.join(', ')}`);
    if (immune.length)
        lines.push(`✗ Imune (×0): ${immune.join(', ')}`);
    return lines.join('\n');
}
function handleWeakToType(attackType, engine) {
    const typeChart = engine.getTypeChart();
    const byAtk = typeChart.get(attackType);
    if (!byAtk)
        return `Bot: Tipo "${typeLabel(attackType)}" não encontrado.`;
    const allPoke = engine.queryAll(`SELECT p.identifier,
            MAX(CASE WHEN pt.slot=1 THEN pt.type_id END) AS type1,
            MAX(CASE WHEN pt.slot=2 THEN pt.type_id END) AS type2
     FROM pokemon p JOIN pokemon_types pt ON p.id = pt.pokemon_id
     WHERE p.identifier NOT LIKE '%_mega%' AND p.identifier NOT LIKE '%_gmax%'
    ...
    for (const poke of allPoke) {
        const defTypes = [poke.type1, poke.type2].filter(Boolean);
        let mult = 1.0;
        for (const dt of defTypes) {
            const v = byAtk.get(dt);
            if (v !== undefined)
        ...
        if (mult >= 4)
            x4.push(displayName(poke.identifier));
        else if (mult >= 2)
            x2.push(displayName(poke.identifier));
    }
    // Type matchups
    const weakTypes = [], resistTypes = [], immuneTypes = [];
    for (const [defType, mult] of byAtk) {
        if (mult >= 2)
            weakTypes.push(typeLabel(defType));
        else if (mult === 0)
            immuneTypes.push(typeLabel(defType));
        else if (mult < 1)
            resistTypes.push(typeLabel(defType));
    }
    const label = typeLabel(attackType);
    const lines = [`Bot: Pokémon fracos contra ${label} (${x4.length + x2.length} total):`];
    if (x4.length)
        lines.push(`  Dupla fraqueza ×4 (${x4.length}): ${x4.slice(0, 8).join(', ')}${x4.length > 8 ? '...' : ''}`);
    lines.push(`  Fraqueza simples ×2 (${x2.length}): ${x2.slice(0, 8).join(', ')}${x2.length > 8 ? '...' : ''}`);
    lines.push('');
    if (weakTypes.length)
        lines.push(`Tipos com fraqueza a ${label}: ${weakTypes.join(', ')}`);
    if (resistTypes.length)
        lines.push(`Tipos que resistem ${label}: ${resistTypes.join(', ')}`);
    if (immuneTypes.length)
        lines.push(`Tipos imunes a ${label}: ${immuneTypes.join(', ')}`);
    return lines.join('\n');
}
function handleBSTThreshold(comparator, threshold, engine) {
    const op = comparator === '>' ? '>=' : '<=';
    const rows = engine.queryAll(`SELECT p.identifier, SUM(ps.value) AS bst
     FROM pokemon p JOIN pokemon_stats ps ON p.id = ps.pokemon_id
     WHERE p.identifier NOT LIKE '%_mega%' AND p.identifier NOT LIKE '%_gmax%'
     GROUP BY p.id HAVING bst ${op} ?
     ORDER BY bst ${comparator === '>' ? 'DESC' : 'ASC'} LIMIT 30`, [threshold]);
    ...
    if (rows.length > 20)
        lines.push(`  ... e mais ${rows.length - 20}`);
    return lines.join('\n');
}
// ---------------------------------------------------------------------------
...
function classifyCompRole(p) {
    const { attack, special_attack: spAtk, speed, hp, defense, special_defense: spDef } = p.baseStats;
    const mainOff = Math.max(attack, spAtk);
    const offBias = attack > spAtk + 25 ? 'físico' : spAtk > attack + 25 ? 'especial' : 'misto';
    if (mainOff >= 115 && speed >= 95)
        return `Sweeper ${offBias} extremo — win condition`;
    if (mainOff >= 90 && speed >= 80)
        return `Attacker ${offBias} — win condition`;
    if (speed <= 50 && mainOff >= 95)
        return `Trick Room Abuser ${offBias} — win condition`;
    if (speed >= 100 && mainOff < 80)
        return 'Speed Control / Pivot — enabler';
    if (hp >= 80 && (defense >= 80 || spDef >= 80) && mainOff < 85)
        return 'Suporte / Pivot — enabler';
    return `Attacker ${offBias}`;
}
function stabCoverage(types, typeChart) {
...
function analyzeNeeds(p, moveset) {
    const condPriority = new Map();
    const abilityInsights = [];
    const passiveInsights = [];
    const moveInsightEntries = [];
    ...
    for (const abilityId of p.abilities) {
        ...
        if (cond) {
            ...
            let note = `${displayName(abilityId)} ativa com ${cond.label}`;
            if (cond.speedMult)
                note += ` → Vel ${p.baseStats.speed} → ${p.baseStats.speed * cond.speedMult}`;
            else if (cond.atkMult)
                note += ` → +50% Atq.Esp.`;
        ...
        const passive = PASSIVE_ABILITY_MAP[abilityId];
        if (passive)
            passiveInsights.push(`${passive.label} — ${passive.note}`);
    }
    for (const moveId of moveset) {
        const mc = MOVE_CONDITION_MAP[moveId];
        if (!mc)
            continue;
        if (mc.minAtk !== undefined && p.baseStats.attack < mc.minAtk)
            continue;
        if (mc.minSpAtk !== undefined && p.baseStats.special_attack < mc.minSpAtk)
            continue;
        // Relevance guard: only highlight when the Pokémon actually benefits from this condition
        const hasAbilityForCondition = p.abilities.some((ab) => ABILITY_CONDITION_MAP[ab]?.condition === mc.condition);
        ...
        if (!hasAbilityForCondition && !hasStabForMove && !isTrAbuser && !selfSetsCondition)
            continue;
    ...
    for (const moveId of moveset) {
        ...
        if (sm.cat === 'physical') {
            if (atkStat < 70)
                continue;
            if (spAtkStat > atkStat + 30)
                continue; // heavily special-biased attacker
        }
        if (sm.cat === 'special') {
            const hasStabOnMove = p.types.includes(sm.type);
            if (hasStabOnMove) {
                if (spAtkStat < 70)
                    continue; // STAB special: standard threshold
            }
            else {
                if (spAtkStat < 90)
                    continue; // non-STAB special: needs high SpAtk to be worth noting
                if (atkStat > spAtkStat + 25)
                    continue; // non-STAB special: not heavily physical-biased
            }
        }
    ...
    if (p.abilities.includes('parental_bond'))
        spreadMoves.length = 0;
...
function getTopHeldItems(p, moveset, isSupport, inSpeedDeadZone, engine, limit = 3) {
    ...
    const allItems = engine.queryAll('SELECT item_id, category, confidence, model_json FROM held_item_effects WHERE confidence >= 0.70 ORDER BY confidence DESC');
    const scored = allItems
        ...
        let model;
        try {
            model = JSON.parse(item.model_json);
        }
        catch {
            model = [];
        }
        const hasFeat = (s) => model.some((m) => m.includes(s));
        switch (item.item_id) {
            case 'choice_band':
                if (offBias === 'physical' && !isSupport)
                    score += 60;
                if (attack >= 120)
                    score += 15;
                if (isSupport || moveset.has('fake_out'))
                    score -= 80;
                break;
            case 'choice_specs':
                if (offBias === 'special' && !isSupport)
                    score += 60;
                if (spAtk >= 120)
                    score += 15;
                if (isSupport || moveset.has('fake_out'))
            ...
            case 'choice_scarf':
                ...
                if (isSupport && !moveset.has('fake_out'))
                    score -= 40;
                if (hasRedirection)
                    score -= 60; // redirectors lose move flexibility with lock-in
                if (moveset.has('fake_out'))
                    score -= 40; // Fake Out users prefer flexible items
                break;
            case 'life_orb':
                if (mainOff >= 100 && !isSupport)
                    score += 55;
                if (mainOff >= 120)
                    score += 15;
                if (isSupport)
            ...
            case 'focus_sash':
                if (isFrail)
                    ...
                    score += 30;
                if (isSupport && (hasRedirection || hasTrickRoom))
                    score += 30;
                break;
            case 'sitrus_berry':
                if (hp >= 80)
            ...
            case 'mental_herb':
                if (isSupport)
                    score += 85;
                if (hasRedirection || hasTrickRoom)
                    score += 25;
                break;
            case 'assault_vest':
                if (!isSupport && hasPivot)
                    score += 40;
                if (spDef <= 80 && mainOff >= 90)
                    score += 20;
                if (isSupport)
                    score -= 25; // can't use support moves with AV
                break;
            case 'rocky_helmet':
            ...
            case 'weakness_policy':
                if (avgBulk >= 50 && mainOff >= 90 && !isSupport)
                    score += 55;
                if (p.abilities.includes('sturdy'))
                    score += 25;
                break;
            case 'safety_goggles':
                score += 20;
            ...
            case 'booster_energy':
                ...
                break;
            case 'leftovers':
                if (isSupport && avgBulk >= 45)
                    score += 35; // support tanks benefit from passive regen
                else if (!isSupport && avgBulk >= 70 && speed <= 80)
                    score += 25; // slow bulky attackers only
                break;
            case 'adrenaline_orb':
                if (hasIntimidateAbility)
                    score -= 20; // the wearer gets Intimidated, not the partner
        ...
        for (const t of p.types) {
            if (hasFeat(`type_hint-${t}`))
                score += 6;
        }
        if (offBias === 'physical' && hasFeat('stat_target-attack'))
            score += 8;
        if (offBias === 'special' && hasFeat('stat_target-special_attack'))
            score += 8;
...
async function handlePairSynergy(idA, idB, engine) {
    const a = engine.getPokemonContext(idA);
    const b = engine.getPokemonContext(idB);
    if (!a)
        return `Bot: ${displayName(idA)} não encontrado.`;
    if (!b)
        return `Bot: ${displayName(idB)} não encontrado.`;
    const movesA = new Set(engine.queryAll('SELECT DISTINCT move_id FROM pokemon_moves WHERE pokemon_identifier = ?', [idA]).map((r) => r.move_id));
    const movesB = new Set(engine.queryAll('SELECT DISTINCT move_id FROM pokemon_moves WHERE pokemon_identifier = ?', [idB]).map((r) => r.move_id));
    const nameA = displayName(idA), nameB = displayName(idB);
    function pairRoleDisplay(p, moves) {
        ...
        if (hasFO && hasIntimidate)
            return 'Suporte — Fake Out + Intimidate (enabler de campo)';
        if (hasRedir)
            return 'Suporte — Redireção (protege parceiro de ataques direcionados)';
        if (hasFO)
            return 'Speed Control / Pivot — enabler';
        if (weatherAb && mainOff < 100)
            return `Setter de ${WEATHER_SETTERS[weatherAb]} / Suporte`;
    ...
    for (const [move, label] of Object.entries(SUPPORT_MOVE_LABELS)) {
        if (movesB.has(move))
            bSupportsA.push(`${nameB} tem ${label}`);
        if (movesA.has(move))
            aSupportsB.push(`${nameA} tem ${label}`);
    }
    // Intimidate
    if (b.abilities.includes('intimidate'))
        bSupportsA.push(`${nameB} tem Intimidate → −1 Atk dos inimigos, protege ${nameA} de dano físico`);
    if (a.abilities.includes('intimidate'))
        aSupportsB.push(`${nameA} tem Intimidate → −1 Atk dos inimigos, protege ${nameB} de dano físico`);
    // Serene Grace + Air Slash flinch lock
    if (b.abilities.includes('serene_grace') && movesB.has('air_slash'))
        bSupportsA.push(`${nameB} Serene Grace + Air Slash → 60% flinch, trava ação inimiga`);
    if (a.abilities.includes('serene_grace') && movesA.has('air_slash'))
        aSupportsB.push(`${nameA} Serene Grace + Air Slash → 60% flinch, trava ação inimiga`);
    // EQ safety
    ...
    if (movesA.has('earthquake') && bImmEQ)
        aSupportsB.push(`${nameA} pode usar Earthquake livremente (${nameB} imune — sem friendly fire)`);
    if (movesB.has('earthquake') && aImmEQ)
        bSupportsA.push(`${nameB} pode usar Earthquake livremente (${nameA} imune — sem friendly fire)`);
    // Weather synergy
    for (const [ab, weather] of Object.entries(WEATHER_SETTERS)) {
        const beneficiaries = WEATHER_BENEFICIARIES[ab] ?? [];
        if (b.abilities.includes(ab) && a.abilities.some((x) => beneficiaries.includes(x)))
            bSupportsA.push(`${nameB} seta ${weather} → ativa habilidade de ${nameA}`);
        if (a.abilities.includes(ab) && b.abilities.some((x) => beneficiaries.includes(x)))
            aSupportsB.push(`${nameA} seta ${weather} → ativa habilidade de ${nameB}`);
    }
...
async function handleSynergySuggestions(identifier, engine) {
    const p = engine.getPokemonContext(identifier);
    if (!p)
        return `Bot: Pokémon "${displayName(identifier)}" não encontrado.`;
    const name = displayName(identifier);
    ...
    const types = p.types.map(typeLabel).join('/');
    const notOwnMega = (r) => !r.identifier.startsWith(identifier + '_mega');
    // Load moveset for condition analysis
    const movesetRows = engine.queryAll('SELECT DISTINCT move_id FROM pokemon_moves WHERE pokemon_identifier = ?', [identifier]);
    ...
    if (isSupport) {
        ...
        for (const ab of p.abilities) {
            ...
            if (sa)
                supportLines.push(`  ⚡ ${displayName(ab)} — ${sa}`);
        }
        ...
        for (const moveId of SUPPORT_PRIORITY) {
            if (!moveset.has(moveId))
                continue;
            const sm = SUPPORT_MOVE_MAP[moveId];
            if (sm)
                supportLines.push(`  ⚡ ${displayName(moveId)} — ${sm}`);
        }
    ...
    if (hasAnyNeed) {
        ...
        if (!isSupport && needs.setupMoves.length > 0) {
            needs.setupMoves.forEach((sm) => sections.push(`  ⚡ ${displayName(sm.move)} — ${sm.desc} → precisa de 1 turno protegido para ativar`));
        }
        // Frailty warning
        ...
        if (needs.primaryCondition && WEATHER_SETTERS[needs.primaryCondition] !== undefined) {
            ...
            if (isSelfSetter) {
                ...
                const settersList = engine.queryAll(`SELECT DISTINCT p2.identifier,
           ...
           WHERE p2.id IN (SELECT pokemon_id FROM pokemon_abilities WHERE ability_id = ?)
             AND p2.identifier != ?
             AND p2.id NOT IN (SELECT DISTINCT from_id FROM pokemon_evolution)
           ORDER BY p2.id LIMIT 8`, [needs.primaryCondition, identifier]).filter((r) => !isNfe(r.identifier) && notOwnMega(r));
    ...
    if (topItems.length > 0) {
        ...
        topItems.forEach((item) => sections.push(`  ${item.icon} ${displayName(item.item_id)} — ${item.note}`));
        sections.push('');
    }
    const candidates = new Map();
    const addScore = (id, typesStr, points, reason) => {
        if (!candidates.has(id))
            candidates.set(id, { identifier: id, types: typesStr, score: 0, contributions: [] });
        const c = candidates.get(id);
        c.score += points;
        c.contributions.push(reason);
    };
    const typeChart = engine.getTypeChart();
    const computeWeaknesses = (defTypes) => {
        const weak = new Set();
        for (const [atkType, byAtk] of typeChart) {
            let mult = 1;
            for (const dt of defTypes)
    ...
    if (needs.primaryCondition && WEATHER_SETTERS[needs.primaryCondition] !== undefined && !isSelfSetter) {
        ...
        engine.queryAll(`SELECT DISTINCT p2.identifier,
              (SELECT GROUP_CONCAT(type_id, '/') FROM pokemon_types WHERE pokemon_id = p2.id ORDER BY slot) AS types
       FROM pokemon p2
       WHERE p2.id IN (SELECT pokemon_id FROM pokemon_abilities WHERE ability_id = ?)
         AND p2.identifier != ?
         AND p2.id NOT IN (SELECT DISTINCT from_id FROM pokemon_evolution)
       LIMIT 300`, [needs.primaryCondition, identifier]).filter((r) => !isNfe(r.identifier) && notOwnMega(r))
            .forEach((r) => addScore(r.identifier, r.types, condSetterScore, `seta ${condLabel}${speedNote} → ativa o potencial completo de ${name}`));
    }
    // Redirect — Follow Me / Rage Powder protect attacker from targeted hits
    if (isAttacker) {
        engine.queryAll(`SELECT DISTINCT p.identifier, GROUP_CONCAT(pt.type_id, '/') AS types
       ...
       WHERE pm.move_id IN ('follow_me', 'rage_powder')
         AND p.identifier != ?
         ${FULLY_EVOLVED_SQL}
       GROUP BY p.id LIMIT 300`, [identifier]).filter((r) => !isNfe(r.identifier) && notOwnMega(r))
            .forEach((r) => addScore(r.identifier, r.types, 25, `redireciona ataques adversários → ${name} ataca sem ser alvo`));
    }
    ...
    if (needsTR) {
       ...
       GROUP BY p.id LIMIT 300`, [identifier]).filter((r) => !isNfe(r.identifier) && notOwnMega(r))
            ...
            if (cp) {
                ...
                if (avgBulk >= 80)
                    survivalNote = ' — bulk alto, alta chance de sobreviver para setar';
                else if (avgBulk < 30)
                    survivalNote = ' — frágil, pode precisar de proteção para setar';
            }
            addScore(r.identifier, r.types, trScore, `seta Trick Room → ${name} age primeiro (Vel base ${p.baseStats.speed})${survivalNote}`);
        });
    }
    ...
    if (inSpeedDeadZone && isAttacker) {
       ...
       WHERE pm.move_id IN ('thunder_wave', 'glare', 'nuzzle')
         ...
         ${FULLY_EVOLVED_SQL}
       GROUP BY p.id LIMIT 300`, [identifier]).filter((r) => !isNfe(r.identifier) && notOwnMega(r))
            .forEach((r) => addScore(r.identifier, r.types, 18, `paralisa o aliado: Vel ${speed} → ~${paraSpeed} — ${name} viável sob Trick Room`));
    }
    // Fake Out — creates a free turn (reduced for TR teams since TR itself gives priority)
    {
       ...
       WHERE pm.move_id = 'fake_out'
         ...
         ${FULLY_EVOLVED_SQL}
       GROUP BY p.id LIMIT 300`, [identifier]).filter((r) => !isNfe(r.identifier) && notOwnMega(r))
            .forEach((r) => addScore(r.identifier, r.types, foPoints, `flinch no 1° turno${setupDesc} (${name} age sem risco)`));
    }
    ...
    if (isAttacker) {
       ...
       WHERE pa.ability_id IN ('armor_tail', 'queenly_majesty')
         ...
         ${FULLY_EVOLVED_SQL}
       GROUP BY p.id LIMIT 300`, [identifier]).filter((r) => !isNfe(r.identifier) && notOwnMega(r))
            .forEach((r) => addScore(r.identifier, r.types, 18, `Armor Tail → bloqueia movimentos de prioridade contra ${name} (Fake Out, Sucker Punch)`));
    }
    ...
    if (isAttacker) {
        ...
        if (terrainAbility) {
         ...
         WHERE pa.ability_id = ?
           AND p.identifier != ?
           ${FULLY_EVOLVED_SQL}
         GROUP BY p.id LIMIT 300`, [terrainAbility, identifier]).filter((r) => !isNfe(r.identifier) && notOwnMega(r))
    ...
    if (isAttacker) {
       ...
       WHERE pa.ability_id = 'intimidate'
         ...
         ${FULLY_EVOLVED_SQL}
       GROUP BY p.id LIMIT 300`, [identifier]).filter((r) => !isNfe(r.identifier) && notOwnMega(r))
            .forEach((r) => addScore(r.identifier, r.types, 15, `Intimidate → −1 Atk nos inimigos ao entrar, protege ${name} de dano físico`));
    }
    ...
    if (moveset.has('earthquake')) {
       ...
       WHERE (p.id IN (SELECT pokemon_id FROM pokemon_types WHERE type_id = 'flying')
         ...
         ${FULLY_EVOLVED_SQL}
       GROUP BY p.id LIMIT 300`, [identifier]).filter((r) => !isNfe(r.identifier) && notOwnMega(r))
            .forEach((r) => addScore(r.identifier, r.types, 20, `imune a Earthquake → ${name} usa EQ livremente, sem friendly fire`));
    }
    ...
    if (isAttacker) {
       ...
       WHERE pm.move_id = 'instruct'
         ...
         ${FULLY_EVOLVED_SQL}
       GROUP BY p.id LIMIT 300`, [identifier]).filter((r) => !isNfe(r.identifier) && notOwnMega(r))
            .forEach((r) => addScore(r.identifier, r.types, 20, `Instruct → ${name} usa o mesmo golpe duas vezes no turno`));
    }
    ...
    for (const [weatherAbility, weatherLabel] of Object.entries(WEATHER_SETTERS)) {
       ...
       LIMIT 300`, [...beneficiaryAbilities, identifier]).filter((r) => !isNfe(r.identifier) && notOwnMega(r))
            .forEach((r) => addScore(r.identifier, r.types, 50, `beneficiado pelo ${weatherLabel} criado por ${name} (stats/velocidade dobram)`));
    }
    ...
    if (hasFakeOut) {
       ...
       LIMIT 300`, [identifier]).filter((r) => !isNfe(r.identifier) && notOwnMega(r))
            .forEach((r) => addScore(r.identifier, r.types, 35, `Fake Out → garante 1° turno livre para ${displayName(r.identifier)} atacar sem resposta`));
    }
    ...
    if (hasIntimidateAbility) {
       ...
       LIMIT 300`, [identifier]).filter((r) => !isNfe(r.identifier) && notOwnMega(r))
            .forEach((r) => addScore(r.identifier, r.types, 20, `Intimidate de ${name} → campo físico mais seguro para ${displayName(r.identifier)} atacar`));
    }
    ...
    if (hasRedirection) {
       ...
       LIMIT 300`, [identifier]).filter((r) => !isNfe(r.identifier) && notOwnMega(r))
            .forEach((r) => addScore(r.identifier, r.types, 40, `Redireção de ${name} → ${displayName(r.identifier)} ataca sem ser alvo (frágil, precisa de cobertura)`));
    }
    // ── Apply penalties ────────────────────────────────────────────────────────
    for (const [id, c] of candidates) {
        ...
        if (shared.length >= 2)
            c.score -= Math.min(8 * shared.length, 18);
    ...
    for (const id of [...candidates.keys()]) {
        ...
        if (megaCand.score > baseCand.score)
...
function handleHeldItemRecommendations(identifier, engine) {
    const p = engine.getPokemonContext(identifier);
    if (!p)
        return `Bot: Pokémon "${displayName(identifier)}" não encontrado.`;
    ...
    const needsTR = speed <= 50;
    const items = engine.queryAll('SELECT item_id, category, description, confidence, model_json FROM held_item_effects WHERE confidence >= 0.72 ORDER BY confidence DESC');
    const scored = items.map((item) => {
        ...
        const hasStat = (s) => model.some((m) => m.includes(s));
        if (offBias === 'physical' && (hasStat('stat_target-attack') || hasStat('modifier_kind-move_power_modifier')))
            score += 0.18;
        if (offBias === 'special' && (hasStat('stat_target-special_attack') || hasStat('modifier_kind-move_power_modifier')))
            score += 0.18;
        if (isSpeedControl && hasStat('stat_target-speed'))
            score += 0.22;
        if (isBulky && (item.category === 'defensive' || hasStat('modifier_kind-hp_recovery_modifier') || hasStat('modifier_kind-damage_taken_modifier')))
            score += 0.12;
        if (isSweeper && item.category === 'offensive')
            score += 0.08;
        if (needsTR && hasStat('stat_target-speed'))
            score -= 0.15;
        for (const t of p.types) {
            if (hasStat(`type_hint-${t}`))
                score += 0.12;
        }
        return { ...item, score };
    });
    ...
    if (!top.length)
        return `Bot: Nenhum held item recomendado para ${displayName(identifier)}.`;
    const types = p.types.map(typeLabel).join('/');
...
let pendingQuizType = null; // set while waiting for difficulty choice
const ALL_TYPES = ['normal', 'fire', 'water', 'electric', 'grass', 'ice', 'fighting', 'poison', 'ground', 'flying', 'psychic', 'bug', 'rock', 'ghost', 'dragon', 'dark', 'steel', 'fairy'];
// Reverse map: PT label (lowercase) → EN id
const TYPE_PT_TO_EN = {};
for (const [en, pt] of Object.entries(TYPE_LABELS))
    TYPE_PT_TO_EN[pt.toLowerCase()] = en;
...
function handleDifficultySelect(type) {
...
function quizSpeedCtrlQuestion(session) {
...
function typeDistractors(variant, seTypes, nveTypes, immuneTypes, correctPool, difficulty) {
    if (difficulty === 1) {
...
function quizTypeEffQuestion(engine, asked, difficulty) {
    const typeChart = engine.getTypeChart();
    // Helper: compute single-type pools
    function computePools(defType) {
        const seTypes = [], nveTypes = [], immuneTypes = [];
        for (const atkType of ALL_TYPES) {
            const mult = typeChart.get(atkType)?.get(defType) ?? 1.0;
            if (mult >= 2.0)
                seTypes.push(TYPE_LABELS[atkType]);
            else if (mult === 0)
                immuneTypes.push(TYPE_LABELS[atkType]);
            else if (mult < 1.0)
                nveTypes.push(TYPE_LABELS[atkType]);
        }
    ...
    if (difficulty === 3) {
        ...
        const wKey = (t1, t2) => `${t1}+${t2}_weakness`;
        const rKey = (t1, t2) => `${t1}+${t2}_resistance`;
        const iKey = (t1, t2) => `${t1}+${t2}_immunity`;
        // Gather all unique dual-type combos from real fully-evolved Pokémon
        const seenKeys = new Set();
        const dualCombos = [];
        for (const p of engine.getAllPokemon()) {
            if (p.types.length !== 2 || nfeIds.has(p.identifier) || p.identifier.includes('mega') || p.identifier.includes('gmax'))
                continue;
            const [t1, t2] = p.types;
            const k = `${t1}+${t2}`;
            if (!seenKeys.has(k)) {
                seenKeys.add(k);
                dualCombos.push({ t1, t2, pokemon: p });
            }
        }
        // Weighted variant pick, filtered to combos not yet asked
        const weaknessAvailD3 = dualCombos.filter(({ t1, t2 }) => {
            const m1 = (atkType) => (typeChart.get(atkType)?.get(t1) ?? 1) * (typeChart.get(atkType)?.get(t2) ?? 1);
        ...
        if (weaknessAvailD3.length === 0 && resistAvailD3.length === 0 && immuneAvailD3.length === 0)
            return null;
        const variantPoolD3 = [];
        for (let i = 0; i < 9; i++)
            if (weaknessAvailD3.length > 0)
                variantPoolD3.push('weakness');
        for (let i = 0; i < 8; i++)
            if (resistAvailD3.length > 0)
                variantPoolD3.push('resistance');
        for (let i = 0; i < 3; i++)
            if (immuneAvailD3.length > 0)
                variantPoolD3.push('immunity');
        ...
        const combinedMult = (atkType) => (typeChart.get(atkType)?.get(t1) ?? 1) * (typeChart.get(atkType)?.get(t2) ?? 1);
        ...
        if (answerPoolD3.length === 0)
            return null; // edge case: no valid answers
    ...
    for (const defType of ALL_TYPES)
        typeData[defType] = computePools(defType);
    ...
    if (weaknessAvail.length === 0 && resistAvail.length === 0 && immuneAvail.length === 0)
        return null;
    const variantPool = [];
    for (let i = 0; i < 9; i++)
        if (weaknessAvail.length > 0)
            variantPool.push('weakness');
    for (let i = 0; i < 8; i++)
        if (resistAvail.length > 0)
            variantPool.push('resistance');
    for (let i = 0; i < 3; i++)
        if (immuneAvail.length > 0)
            variantPool.push('immunity');
...
function quizMoveTypeQuestion(engine, asked, difficulty) {
...
function quizPokemonStatsQuestion(engine, asked, difficulty) {
    ...
    const allFe = engine.getAllPokemon().filter((o) => o.identifier !== p.identifier &&
    ...
    if (difficulty === 3 && p.types.length > 0) {
        ...
        if (sameType.length >= 2)
            distractorPool = shuffleArray([...sameType, ...allFe.slice(1, 4)]);
    }
...
function startQuizSession(type, difficulty, engine) {
...
function nextQuizQuestion(engine) {
    if (!quizSession)
        return 'Bot: Nenhum quiz ativo. Digite "quiz" para escolher um modo.';
    const diff = quizSession.difficulty;
    let q = null;
    if (quizSession.type === 'type_eff')
        q = quizTypeEffQuestion(engine, quizSession.asked, diff);
    else if (quizSession.type === 'move_type')
        q = quizMoveTypeQuestion(engine, quizSession.asked, diff);
    else if (quizSession.type === 'speed_ctrl')
        q = quizSpeedCtrlQuestion(quizSession);
    else
        q = quizPokemonStatsQuestion(engine, quizSession.asked, diff);
    if (!q) {
...
function fuzzyMatchTypeName(userInput, options) {
    ...
    for (const opt of options) {
        ...
        if (levenshtein(u, o) <= threshold)
            return true;
        const enId = TYPE_PT_TO_EN[opt] ?? '';
        if (enId && levenshtein(u, enId) <= threshold)
            return true;
    }
    return false;
}
function checkQuizAnswer(raw, engine) {
    if (!quizSession)
        return 'Bot: Nenhum quiz ativo.';
    ...
    if (letterIdx >= 0 && quizSession.choices.length === 4) {
        ...
        correct = validOptions.some((opt) => {
            if (opt === userAnswer)
                return true;
            if (opt.includes(userAnswer) || userAnswer.includes(opt))
                return true;
            const enId = TYPE_PT_TO_EN[opt] ?? '';
        ...
        if (!correct)
            correct = fuzzyMatchTypeName(userAnswer, quizSession.answer.split('|'));
    }
    else if (quizSession.type === 'move_type') {
        ...
        if (!correct && enId && t.includes(enId))
            correct = true;
        if (!correct)
            correct = fuzzyMatchTypeName(t, [quizSession.answer]);
    }
    else if (quizSession.type === 'speed_ctrl') {
...
function vbPendingActions(state, engine) {
    const result = [];
    const userTeam = state.teams[0];
    const oppTeam = state.teams[1];
    const firstOpp = oppTeam.active.find((p) => p && !p.fainted);
    for (let s = 0; s < 2; s++) {
        const poke = userTeam.active[s];
        if (!poke || poke.fainted)
            continue;
        const moves = poke.config.moves
            .filter((mid) => {
            if (poke.choiceLocked && poke.choiceLocked !== mid)
                return false;
            if ((poke.ppRemaining[mid] ?? 1) <= 0)
                return false;
            return true;
        })
            .map((mid) => {
        ...
        result.push({ slot: s, actorUid: poke.uid, actorName: battleDisplayName(poke.identifier), moves, switches: bench });
    }
    return result;
}
...
function handleBattleInitJson(jsonStr, engine) {
    try {
        const req = JSON.parse(jsonStr);
        const aiPokemon = (req.mode === 'custom' && req.enemy) ? req.enemy.pokemon : buildAITeam(engine);
        if (aiPokemon.length < 4)
            return JSON.stringify({ type: 'error', message: 'Não foi possível montar o time da IA.' });
        const aiCfg = { pokemon: aiPokemon };
        const sim = new engine_1.BattleSimulator(engine);
        const ai = new engine_1.BattleAI(sim, engine);
...
function handleBattleActJson(jsonStr, engine) {
    if (!visualBattleSession)
        return JSON.stringify({ type: 'error', message: 'Nenhuma batalha ativa.' });
    try {
        const req = JSON.parse(jsonStr);
            ...
            for (let s = 0; s < 2; s++) {
                ...
                if (!slot || slot.fainted) {
                    ...
                    if (hasBench) {
                        ...
                        if (pi >= 0)
                            sim.forceSwitch(result.state, 1, s, pi);
                    }
                }
            }
        }
...
function handleBattleSwitchJson(jsonStr, engine) {
    if (!visualBattleSession)
        return JSON.stringify({ type: 'error', message: 'Nenhuma batalha ativa.' });
    try {
        const req = JSON.parse(jsonStr);
...
function handleStudyTypes(engine) {
    const typeChart = engine.getTypeChart();
    const lines = ['Bot: ── Chart de Tipos — Cobertura Ofensiva ──', ''];
    for (const atkType of ALL_TYPES) {
        const byAtk = typeChart.get(atkType);
        if (!byAtk)
            continue;
        ...
        if (nve.length)
            lines.push(`${' '.repeat(11)}NVE: ${nve.join(', ')}`);
        if (imm.length)
            lines.push(`${' '.repeat(11)}Imune: ${imm.join(', ')}`);
    }
    return lines.join('\n');
}
function handleStudyRoles() {
    return [
...
function detectIntent(t) {
    ...
    if (/\bvs\b|\bversus\b|comparar|comparando|diferenca.*entre|quem.*melhor.*entre/.test(t))
        return 'compare';
    // Rankings
    if (/mais.*veloc|mais.*rapid|mais.*veloz|veloc.*rank|speed.*rank/.test(t))
        return 'ranking_speed';
    if (/ranking|top\s*\d/.test(t)) {
        const s = extractStatNameFromText(t);
        return s === 'speed' ? 'ranking_speed' : 'ranking_stat';
    }
    if (/maior.*\b(hp|ataque|defesa|spatk|spdef|bst)\b|mais.*\b(hp|ataque|defesa)\b|top.*em.*(hp|ataque|defesa|velocidade|bst)/.test(t))
        return 'ranking_stat';
    // BST threshold
    if (/bst.*(acima|abaixo|de|acima de|abaixo de)|pokemon.*bst|bst.*pokemon/.test(t))
        return 'bst_threshold';
    // Competitive profile
    if (/perfil.*comp|papel.*comp|como.*usar.*comp|vis[aã]o.*comp/.test(t))
        return 'competitive_profile';
    if (/\bperfil\b|\bpapel\b/.test(t))
        return 'competitive_profile';
    if (/me\s+fala\s+(sobre|do|de|sobre\s+o)|me\s+explica\b|\bexplica\b|\banalisa\b|\banalise\b/.test(t))
        return 'competitive_profile';
    if (/\bcomo\s+usar\b/.test(t))
        return 'competitive_profile';
    // "é/e X bom" — PT-BR informal assessment (é normalizes to e via NFD)
    if (/\be\s+(o\s+|a\s+)?\w+\s+bom\b|\bdevo\s+usar\b|\bvale\s+(a\s+pena\s+)?usar\b/.test(t))
        return 'competitive_profile';
    // Pair synergy (BEFORE synergy patterns — more specific; requires 2 pokémon + explicit connector)
    if (/dupla\b/.test(t) && extractTwoPokemon(t) !== null)
        return 'pair_synergy';
    // Natural language pair: "X e Y juntos", "X junto com Y", "X e Y funcionam juntos"
    if (/\bjunto[s]?\b/.test(t) && extractTwoPokemon(t) !== null)
        return 'pair_synergy';
    // EN pair patterns: "how good is X with Y", "how well does X work with Y", "X with Y" (two Pokémon)
    if (/how\s+good\s+is\b|how\s+well\s+does\b/.test(t) && extractTwoPokemon(t) !== null)
        return 'pair_synergy';
    if (/\bwith\b/.test(t) && extractTwoPokemon(t) !== null)
        return 'pair_synergy';
    // Synergy
    if (/sinergi|parceir|combina.*com|quem.*bom.*junto|quem.*junto.*com/.test(t))
        return 'synergy';
    if (/com\s+quem|quem\s+vai.*bem|vai\s+bem\s+com|funciona.*junto|funciona.*com\b/.test(t))
        return 'synergy';
    if (/dupla\b/.test(t))
        return 'synergy';
    // Items
    if (/held.*item|item.*para|iten[s]?.*para|melhor.*item|melhor.*iten|equipamento|item.*recomend|iten[s]?.*recomend/.test(t))
        return 'held_item';
    // Bad matchup analysis — user asks about threats to their own Pokémon (BEFORE counter + weak_to_type)
    if (/dificuldade.*usando|usando.*dificuldade|dif[ií]cil.*usar/.test(t))
        return 'bad_matchup';
    if (/contra quem.*sofre|contra quem.*perde|quem.*derrota\b/.test(t))
        return 'bad_matchup';
    if (/fraqueza(s)?\s+do\b|ponto(s)?\s+fraco(s)?\s+do\b/.test(t) && !hasTypeToken(t))
        return 'bad_matchup';
    if (/counters?\s+(do|para|de)\b|o que\s+countera|quem\s+countera/.test(t))
        return 'bad_matchup';
    if (/matchup\s+ruim|mau\s+matchup|bad\s+matchup/.test(t))
        return 'bad_matchup';
    if (/o que\s+amea[cç]a\b|quem\s+amea[cç]a\b/.test(t))
        return 'bad_matchup';
    // Type weakness queries (BEFORE counter — "quem e fraco contra X" would match counter's "quem.*contra")
    if (/fraco.*contra|fraqueza.*a |fraqueza.*ao|fraqueza.*do tipo|quem.*e.*fraco|quem.*fraco|quem.*sofre|pokemon.*fraco|fracos.*a /.test(t) && hasTypeToken(t))
        return 'weak_to_type';
    // Type coverage (EN) — checked BEFORE counter to avoid "what beats dragon type" → counter
    if (/what\s+beats?\s+\w+\s+type\b|what\s+is\s+super\s+effective\s+against|what\s+type.*super\s+effective/.test(t))
        return 'type_coverage';
    if (/cobertura.*tipo|tipo.*cobertura|o que.*bate.*tipo|forte.*contra.*tipo|o que.*vence.*tipo/.test(t))
        return 'type_coverage';
    // Counter
    if (/quem.*bate|quem.*vence|quem.*contra|counter.*para|como.*derrotar|como.*bater/.test(t))
        return 'counter';
    if (/how\s+to\s+beat\b|how\s+to\s+counter\b|what\s+counters?\b/.test(t))
        return 'counter';
    if (/what\s+beats?\s+\w+(?:\s+\w+)?\?/.test(t) && !hasTypeToken(t))
        return 'counter';
    if (/pokemon.*(?:do|de|com).*tipo|(?:do|de|com).*tipo.*pokemon|listar.*tipo|que.*tipo/.test(t))
        return 'type_query';
    // Generation — region names map to specific generations
    if (/lend[aá]r|legend[aá]r|m[ií]tico|mythic/.test(t))
        return 'legendary_query';
    if (/(ger[ae]?[cç][aã]o?|gen)\s*\d/.test(t))
        return 'generation_query';
    if (/\b(kanto|johto|hoenn|sinnoh|unova|kalos|alola|galar|paldea)\b/.test(t))
        return 'generation_query';
    // Evolution
    if (/evolu[çc]|como.*evol|cadeia.*evol|quando.*evol/.test(t))
        return 'evolution';
    // Movelist / ability list
    if (/que.*golpe.*aprende|golpe.*aprende|movelist|moveset.*d[eo]|golpes.*do|ataques.*do|golpes.*aprende/.test(t))
        return 'movelist';
    if (/habilidade.*de|ability.*de|que.*habilidade.*tem|habilidades.*do/.test(t))
        return 'ability_info_pokemon';
    // VGC / Doubles format overview — must come before generic "como funciona" catch-all
    if (/\bvgc\b/.test(t) && /como|o\s+que|explica|regras?|formato|funciona|e\s+o/.test(t))
        return 'study_vgc';
    if (/\bdoubles?\b/.test(t) && /como|o\s+que|regras?|formato|funciona/.test(t))
        return 'study_vgc';
    if (/formato.*pokemon|pokemon.*formato|regras.*batalha|batalha.*dupla/.test(t))
        return 'study_vgc';
    // Specific move or ability info
    if (/o que.*faz|para que serve|como.*funciona/.test(t)) {
        if (/speed\s*control|win\s*condi[ct]|tailwind|trick\s*room|fake\s*out|redirect|terrain|pivot\s+vgc|lead\s+vgc/.test(t))
            return 'study_mechanics';
        if (/habilidade|ability|passiva/.test(t))
            return 'ability_info';
        if (/golpe|move|ataque/.test(t))
            return 'move_info';
        // Try ability first — avoids substring collisions (e.g. "protosynthesis" contains "synthesis")
        if (extractAbilityName(t))
            return 'ability_info';
        if (extractMoveName(t))
            return 'move_info';
    ...
    if (/info|dados|detalhe|stat|base stat|atributo/.test(t))
        return 'detail';
    // Quiz
    {
        ...
        if (/\bquiz\s+(de\s+)?(tipos?|type)\b/.test(t)) {
            if (HARD_RE.test(t))
                return 'quiz_types_hard';
            if (MED_RE.test(t))
                return 'quiz_types_med';
            if (EASY_RE.test(t))
                return 'quiz_types_easy';
            return 'quiz_types';
        }
        if (/\bquiz\s+(de\s+)?(golpes?|moves?|ataques?)\b/.test(t)) {
            if (HARD_RE.test(t))
                return 'quiz_moves_hard';
            if (MED_RE.test(t))
                return 'quiz_moves_med';
            if (EASY_RE.test(t))
                return 'quiz_moves_easy';
            return 'quiz_moves';
        }
        if (/\bquiz\s+(de\s+)?(pokemon|pokemons?)\b/.test(t)) {
            if (HARD_RE.test(t))
                return 'quiz_pokemon_hard';
            if (MED_RE.test(t))
                return 'quiz_pokemon_med';
            if (EASY_RE.test(t))
                return 'quiz_pokemon_easy';
            return 'quiz_pokemon';
        }
        if (/\bquiz\s+(de\s+)?(velocidade|speed|controle|prioridade|tailwind|trick\s*room|mecanicas?|speed\s*ctrl)\b/.test(t)) {
            if (HARD_RE.test(t))
                return 'quiz_speed_hard';
            if (MED_RE.test(t))
                return 'quiz_speed_med';
            if (EASY_RE.test(t))
                return 'quiz_speed_easy';
            return 'quiz_speed';
        }
    }
    if (/^\s*quiz\s*$/.test(t) || /\bmode\s+quiz\b|\bquiz\s+mode\b/.test(t))
        return 'quiz_lobby';
    if (/\bquiz\b/.test(t))
        return 'quiz_lobby'; // bare quiz keyword → lobby
    if (/estudar?\s+tipos?|chart\s+de\s+tipos?|tabela\s+de\s+tipos?|tipo.*efetividade/.test(t))
        return 'study_types';
    if (/pap[eé]is?\s+(competitiv|vgc)|roles?\s+vgc|fun[cç][oõ]es?\s+competitiv/.test(t))
        return 'study_roles';
    if (/mecanicas?\s+vgc|como\s+funciona\s+(tailwind|trick\s*room|redirect|fake\s+out|terrain)|veloc.*vgc|speed\s+control\s+vgc/.test(t))
        return 'study_mechanics';
    if (/como\s+funciona(m)?\s+(o\s+|a\s+)?(meta|speed\s+control|win\s+con|setup|pivot|lead\s+vgc|back\s+vgc)/.test(t))
        return 'study_mechanics';
    ...
    if (/sou\s+(um\s+)?(novo|iniciante)|novo\s+(no|ao|em|jogador|player)|nunca\s+(joguei|jogar)|como\s+come[cç]ar|por\s+onde\s+come[cç]ar/.test(t))
        return 'onboarding';
    if (/me\s+explica\s+(o\s+)?(b[aá]sico|basicos?|as\s+regras|conceitos|o\s+jogo)|explica\s+(o\s+b[aá]sico|conceitos\s+b[aá]sicos)/.test(t))
        return 'onboarding';
    if (/quero\s+(aprender|entender|saber)\s+(pokemon\s+competitiv|vgc|como\s+funciona\s+o\s+vgc)/.test(t))
        return 'onboarding';
    if (/(?:explain|teach\s+me|i(?:'m|\s+am)\s+(?:new|a\s+beginner)|beginner|just\s+started?|never\s+played?)\s+(?:pokemon|vgc|competitive)/.test(t))
        return 'onboarding';
    return 'unknown';
}
async function handleNLText(text, engine) {
    ...
    switch (intent) {
        ...
        case 'counter':
            ...
            if (/trick\s*room/.test(t))
                return 'Bot: Para bater Trick Room: use Pokémon rápidos com Taunt (bloqueia setup), Imprison (bloqueia TR), ou golpes de prioridade (Fake Out, Sucker Punch). Equipes rápidas com Tailwind também superam a janela de TR.';
            if (!pokemonId)
                return 'Bot: Qual Pokémon? Ex: "quem bate Garchomp"';
            return handleCounterQuery(pokemonId, engine);
        case 'compare': {
            const pair = extractTwoPokemon(t);
            if (!pair)
                return 'Bot: Especifique dois Pokémon. Ex: "Charizard vs Blastoise"';
            return handleCompareQuery(pair[0], pair[1], engine);
        }
        case 'evolution':
            if (!pokemonId)
                return 'Bot: Qual Pokémon? Ex: "como evolui Eevee"';
            return handleEvolutionChain(pokemonId, engine);
        case 'movelist':
            if (!pokemonId)
                return 'Bot: Qual Pokémon? Ex: "golpes que Togekiss aprende"';
            return handleMovelist(pokemonId, engine);
        case 'ability_info_pokemon':
            if (!pokemonId)
                return 'Bot: Qual Pokémon? Ex: "habilidades do Garchomp"';
            return handleAbilityInfoForPokemon(pokemonId, engine);
        case 'ability_info': {
            const abilityId = extractAbilityName(t);
            if (!abilityId && pokemonId)
                return handleAbilityInfoForPokemon(pokemonId, engine);
            if (!abilityId)
                return 'Bot: Qual habilidade? Ex: "o que faz Intimidate"';
            return handleAbilityInfo(abilityId, engine);
        }
        case 'move_info': {
            const moveId = extractMoveName(t);
            if (!moveId)
                return 'Bot: Qual golpe? Ex: "o que é Earthquake"';
            return handleMoveInfo(moveId, engine);
        }
        case 'type_query': {
            const typeId = extractTypeName(t);
            if (!typeId)
                return 'Bot: Qual tipo? Ex: "pokemon do tipo Fogo"';
        ...
        case 'generation_query': {
            ...
            if (!gen)
                return 'Bot: Qual geração? Ex: "pokemon da geração 3"';
        ...
        case 'type_coverage': {
            const typeId = extractTypeName(t);
            if (!typeId)
                return 'Bot: Qual tipo? Ex: "o que bate o tipo Pedra"';
            return handleTypeCoverage(typeId, engine);
        }
        case 'weak_to_type': {
            const typeId = extractTypeName(t);
            if (!typeId)
                return 'Bot: Qual tipo? Ex: "quem é fraco contra Fogo"';
...
// Command handler
// ---------------------------------------------------------------------------
async function handleCommand(input, engine) {
    const raw = input.trim();
    if (raw === '__PING__')
        return `pong v${BRIDGE_VERSION}`;
    if (raw === '__RESET__') {
        quizSession = null;
        pendingQuizType = null;
        battleSession = null;
    ...
    if (pendingQuizType !== null) {
        ...
        if (/^(f[aá]cil|easy|1)$/.test(d))
            return startQuizSession(pendingQuizType, 1, engine);
        if (/^(m[eé]di[oa]?|medium|2)$/.test(d))
            return startQuizSession(pendingQuizType, 2, engine);
        if (/^(dif[ií]cil|hard|3)$/.test(d))
            return startQuizSession(pendingQuizType, 3, engine);
        if (/^(quiz|cancelar|cancel|voltar|menu)$/.test(d)) {
            pendingQuizType = null;
            return handleQuizLobby();
        }
    ...
    if (raw.startsWith('__BATTLE_INIT_JSON__:'))
        return handleBattleInitJson(raw.slice('__BATTLE_INIT_JSON__:'.length), engine);
    if (raw.startsWith('__BATTLE_ACT_JSON__:'))
        return handleBattleActJson(raw.slice('__BATTLE_ACT_JSON__:'.length), engine);
    if (raw.startsWith('__BATTLE_SWITCH_JSON__:'))
        return handleBattleSwitchJson(raw.slice('__BATTLE_SWITCH_JSON__:'.length), engine);
    if (raw === '')
        return 'Bot: Digite uma pergunta para continuar.';
...
async function main() {
    ...
    let engine;
    try {
        engine = new engine_1.DeterministicEngine(DB_PATH);
    }
    catch (e) {
    ...
    function bridgeCleanup() {
        if (pyProcess) {
            try {
                pyProcess.stdin.write(JSON.stringify({ type: 'exit' }) + '\n');
            }
            catch { /* ignore */ }
        }
        engine.close();
