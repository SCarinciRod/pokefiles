...
exports.SELF_TARGET_MOVES = exports.SPREAD_MOVES = exports.TYPE_RESIST_BERRY = exports.ITEM_TYPE_BOOST = exports.ABILITY_REGISTRY = void 0;
exports.getStatusMoveEffect = getStatusMoveEffect;
// ---------------------------------------------------------------------------
...
function setWeather(state, weather, turns, log) {
...
function setTerrain(state, terrain, turns, log) {
...
function applyStatDrop(target, stat, stages, ctx) {
    ...
    if (delta !== 0)
        ctx.log(`${target.identifier}: ${stat} ${delta > 0 ? '+' : ''}${delta}!`);
    // Competitive / Defiant: +2 to special attack or attack when a stat is dropped by opponent
    if (delta < 0) {
        if (target.currentAbility === 'competitive')
            target.boosts.special_attack = Math.min(6, target.boosts.special_attack + 2);
        if (target.currentAbility === 'defiant')
            target.boosts.attack = Math.min(6, target.boosts.attack + 2);
    }
}
...
exports.ABILITY_REGISTRY = {
    ...
    sand_rush: {
        on_damage_calc: (ctx) => {
            if (ctx.state.field.weather === 'sandstorm')
                ctx.speedMult *= 2;
        },
    },
    slush_rush: {
    ...
    grass_pelt: {
        on_damage_calc: (ctx) => {
            if (ctx.state.field.terrain === 'grassy')
                ctx.speedMult *= 1.5;
        },
    },
    speed_boost: {
    ...
    unburden: {
        on_damage_calc: (ctx) => {
            if (ctx.self.itemConsumed && ctx.self.config.item !== null)
                ctx.speedMult *= 2;
        },
    },
    ...
    multiscale: {
        on_damage_calc: (ctx) => {
            if (ctx.self.currentHp === ctx.self.maxHp)
                ctx.defenseMult *= 0.5;
        },
    },
    shadow_shield: {
...
// ---------------------------------------------------------------------------
exports.SELF_TARGET_MOVES = new Set([
...
function getStatusMoveEffect(moveId) {
    switch (moveId) {
