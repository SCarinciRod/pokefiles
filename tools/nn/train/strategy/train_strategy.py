...
ALL_ROLES = [
    "physical_sweeper", "special_sweeper", "setup_sweeper",
    "physical_wall", "special_wall", "tank", "lead",
    "hazard_setter", "hazard_remover", "pivot_volt_turn",
    "cleric", "support_utility", "balanced",
]
...
def extract_synergy_features(record: dict) -> tuple[list[float], float]:
    ...
    types_a = type_onehot(inp.get("types_a"))
    types_b = type_onehot(inp.get("types_b"))
    score = float(out.get("score", 0.0))
    score_norm = min(1.0, max(0.0, score / 1000.0)) if score > 1.0 else score

    feat = types_a + types_b + [0.0, 0.0]
    return feat, score_norm
