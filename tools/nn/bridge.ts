...
// ---------------------------------------------------------------------------
const ABILITY_TYPE_MODIFIERS: Record<string, Array<{ type: string; mult: number }>> = {
...
function buildEvolutionData(pokemonId: number, pokemonIdentifier: string, engine: DeterministicEngine) {
  ...
  interface FormRow { form_id: number; base_id: number; }
  ...
  const megaForms = engine.queryAll<FormRow>(
    `SELECT form_id, base_id FROM pokemon_forms WHERE form_type = 'mega' AND base_id IN (${idsArray})`
  );

  if (chainEdges.length === 0 && megaForms.length === 0) return { members: [], transitions: [] };
  ...
  for (const mega of megaForms) {
    const baseRow = engine.queryAll<PRow>('SELECT identifier FROM pokemon WHERE id = ?', [mega.base_id])[0];
    const formRow = engine.queryAll<PRow>('SELECT identifier FROM pokemon WHERE id = ?', [mega.form_id])[0];
    if (baseRow && formRow) {
...
function buildDetailEntry(p: PokemonContext, typeChart: Map<string, Map<string, number>>, engine: DeterministicEngine) {
  ...
  const ability_options = p.abilities.map((abilityId) => {
    ...
    if (mods.length > 0) {
      const aw: typeof weaknesses = [];
      const ar: typeof weaknesses = [];
      const ai: typeof immunities = [];
      for (const atkType of ALL_ATTACK_TYPES) {
        const byAtk = typeChart.get(atkType);
        let mult = 1.0;
        ...
        if (mult > 1.0) aw.push({ type: atkType, type_label: typeLabel(atkType), multiplier: `×${mult}`, multiplier_value: mult });
        else if (mult === 0.0) ai.push({ type: atkType, type_label: typeLabel(atkType) });
        else if (mult < 1.0) ar.push({ type: atkType, type_label: typeLabel(atkType), multiplier: `×${mult}`, multiplier_value: mult });
      }
