...
function loadTacticalCatalog(filePath) {
  ...
  for (const line of lines) {
    ...
    if (seedMatch) { addToMultiMap(seedRolesByMove, sanitizeAtom(seedMatch[1]), sanitizeAtom(seedMatch[2])); continue; }
    ...
    if (expandMatch) {
      ...
      if (parent !== 'role' && child !== 'role') addToMultiMap(expandRoleMap, parent, child);
    }
  }
